...
def systemUI():
    if platform.system() == 'Windows':
        call([r'.\temp\aria2c', '--file-allocation=none', '--continue=true', '--retry-wait=5', '-m 0', '-x 8', '-s 8', 'https://07th-mod.com/rikachama/Tsumihoroboshi-UI.7z'])
    else:
        call([r'.\temp\aria2c', '--file-allocation=none', '--continue=true', '--retry-wait=5', '-m 0', '-x 8', '-s 8', 'https://07th-mod.com/rikachama/Tsumihoroboshi-UI_UNIX.7z'])       
...
def cleanCG():
    ...
    old_CompiledUpdate = "./HigurashiEp06_Data/StreamingAssets/CompiledUpdateScripts"
    
    if os.path.isdir(old_CG) and os.path.isdir(old_CGAlt):
        shutil.rmtree(old_CG)
        shutil.rmtree(old_CGAlt)

    for file in os.listdir("./HigurashiEp06_Data/StreamingAssets/CompiledUpdateScripts"):
