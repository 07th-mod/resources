...
echo.
if not exist "Umineko1to4_backup.exe" move /Y Umineko1to4.exe Umineko1to4_backup.exe || echo INFO - Couldn't backup exe - continuing install anyway
echo.
...
:downloadLoop    
.\temp\aria2c --console-log-level=error --file-allocation=none --continue=true --check-integrity=true --max-concurrent-downloads=1 --retry-wait 5 -x 8 --follow-metalink=mem https://github.com/07th-mod/resources/raw/master/umineko-question-1080p/umi_full.meta4 --dir=temp || (timeout /t 3 && goto :downloadLoop)
...
.\temp\7za.exe x temp\Umineko-Graphics-1080p.7z -aoa -bsp1 || echo ERROR during graphics extraction. Check the game files are not in use. && goto :installFailed
...
.\temp\7za.exe x temp\*ADV*.7z -aoa -bsp1 || echo ERROR during updates extraction. Check the game files are not in use. && goto :installFailed
